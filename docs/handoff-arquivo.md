...
# Arquivo de Handoff — histórico completo

> **Este arquivo é append-only e não é lido no início de uma sessão.**
> O handoff vivo é [`CURRENT-STATE.md`](./CURRENT-STATE.md), que contém apenas o estado
> corrente. Quando uma rodada deixa de ser a atual, seu relato é movido para cá.
>
> Consulte este arquivo sob demanda — para entender *por que* uma decisão antiga foi tomada,
> rastrear quando um comportamento surgiu, ou recuperar o detalhe de um micro-ticket já
> fechado. Não é necessário lê-lo por inteiro; use busca.
