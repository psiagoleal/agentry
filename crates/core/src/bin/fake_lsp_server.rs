...
//! Fixture de teste (MT-23, ADR-0013): um "language server" mínimo, só o
//! suficiente para o teste de ciclo de vida de `LspClient`
//! (`start` → `initialize` → `shutdown`) exercitar spawn + JSON-RPC real
//! sobre stdio, sem depender de nenhum *language server* de verdade
//! instalado no ambiente de CI (ADR-0013 proíbe o `agentry` de empacotar
//! um). Não é parte do produto — só um binário auxiliar de teste, spawnado
//! via `env!("CARGO_BIN_EXE_fake_lsp_server")` a partir dos testes de
//! `context::lsp::client`.
//!
//! Responde `initialize`/`shutdown` com sucesso trivial e encerra ao
//! receber a notificação `exit` — mesma técnica de framing (`lsp_server::Message`)
